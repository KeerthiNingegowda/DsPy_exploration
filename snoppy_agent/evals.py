...
def extract_tools_from_trace(pred):
    ...
    """ Extract tools from the predicted response"""
...
def ground_truth_for_accuracy_evaluation(
    query_context: str, user_preferences: dict
) -> str:
...
def llm_as_a_judge_accuracy_eval(
    query: str,
    ground_truth: str,
...
def prep_data(data: list, user_preferences: dict, chat_history: str = "") -> list:
    ...
    return [
        dspy.Example(
            d, user_preferences=user_preferences, chat_history=chat_history
        ).with_inputs("query", "user_preferences", "chat_history")
        for d in data
    ]
...
def build_evaluator(devset: list, path_to_save: str):
    ...
    """ Create and return evaluator object for tuning"""
